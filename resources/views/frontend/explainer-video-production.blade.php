@extends('partials.frontend.app')

@section('frontend-content')
    <style>
        .btn-back{
            background-color: #DD1C4F !important;
        }
        .arrow-bar-1, .arrow-bar-2 {
            background-color: #fff;
        }
    </style>
    <div class="container-fluid outerwrapper">
        <div class="col-md-7 col-md-push-4">
            <br class="visible-xs"/>
            <img src="{{asset('img/Programmer Needs-69.svg')}}" height="94">
            <h1 class="text-lightblue">EXPLAINER VIDEO PRODUCTION</h1>
            <h2>Why do we need videos for business?</h2>
            <br/>

            <img src="{{asset('img/Programmer Needs-70.jpg')}}" class="img-full">
            <br/>
            <br/>
            <h4>If a picture is worth a thousand words, what more for a video?</h4>
            <p>
                We can listen and see the pictures moving and then we get emotional with it. This is somewhat we called
                “engaging the audiences”. But why must a video and instead of writing or other means of communication?
                Take a look at some successful movies like “The Lord of the Rings” and “Harry Potter” and many others
                are books that had turned into blockbuster movies. People would prefer to have all senses on our body to
                be satisfied, at least, visual and auditory senses at the same time.</p>

            <p>You may have a wonderful story to share about your business and its success. For example, the targeted
                audiences would love to know how a product is made and the process, ingredients used and its safety.
                Consumers are more vigilant nowadays compared to the past generation due to health awareness and the
                audience demands more and better visual quality following the advancement in technology. </p>
            <br/>
            <br/>
            <h2>Here are some good points: </h2>

            <br/>
            <div class="row">
                <div class="col-sm-6">
                    <div class="row">
                        <div class="col-xs-5">
                            <img src="{{asset('img/Programmer Needs-71.svg')}}" class="img-full">
                        </div>
                    </div>
                    <h2 class="text-lightblue">Cost Saving</h2>
                    <p>In fact, this explainer video is the best video production that you can control the content. To
                        compare with real shoot videos, motion graphic video can save your cost on talents, props,
                        shooting crew & equipment rental fees. </p>
                    <br/>
                </div>

                <div class="col-sm-6">
                    <div class="row">
                        <div class="col-xs-5">
                            <img src="{{asset('img/Programmer Needs-72.svg')}}" class="img-full">
                        </div>
                    </div>
                    <h2 class="text-lightblue">Branding Image</h2>
                    <p>In order to keep your brand up in the market, professional and trendy you must have an explainer
                        video to keep up with competitors and also make your customers feeling proud to have chosen
                        you.</p>
                    <br/>
                </div>
            </div>
            <br/>
            <br/>
            <div class="row">
                <div class="col-sm-6">
                    <div class="row">
                        <div class="col-xs-5">
                            <img src="{{asset('img/Programmer Needs-73.svg')}}" class="img-full">
                        </div>
                    </div>
                    <h2 class="text-lightblue">Simplicity</h2>
                    <p>It’s a superb useful marketing tool nowadays. Firstly, it is not wordy but simple to understand
                        as the explainer video goes along to clarify your intended idea. With that at hand, you can
                        simply use it to do an attractive fabulous presentation to your customers instead of the old
                        powerpoint slides show anymore. The marketing people can do away their manual script and present
                        with confidence, this is really a godsend thing to help a difficult explained product. We are
                        pretty sure the customers would not doze off during your presentation but delighted by your
                        video. ternet and boost your website rankings because people stay on your website watching and
                        eventually convert into sales. According to survey done by Video Rascal a product that has an
                        explainer video accompanied will sell better. </p>
                    <br/>
                </div>

                <div class="col-sm-6">
                    <div class="row">
                        <div class="col-xs-5">
                            <img src="{{asset('img/Programmer Needs-74.svg')}}" class="img-full">
                        </div>
                    </div>
                    <h2 class="text-lightblue">Drive Traffic & Sales</h2>
                    <p>I bet you have seen many of these explainer videos on social media like Facebook and noticed how
                        many thousands of people have watched, commented and forwarded to friends without limit. You too
                        wish to have one explainer video that will go viral on the internet and boost your website
                        rankings because people stay on your website watching and eventually convert into sales.
                        According to survey done by Video Rascal a product that has an explainer video accompanied will
                        sell better. </p>
                    <br/>
                </div>
            </div>

            <h2 class="text-lightblue">Why wait, get in touch with us !</h2>
            <p>This explainer video is proven to be very effective and popular among many successful businesses in this
                era. Find out more about our explainer video production and the following are three types of explainer
                videos available to suit your need:</p>
            <div class="row">
                <div class="col-sm-4">
                    <img src="{{asset('img/Programmer Needs-75.jpg')}}" class="img-full m-t-md">
                </div>
                <div class="col-sm-8">
                    <h2>Motion Graphic video</h2>
                    <p>Explain with info graphics & simple character, suitable for corporate profile & product
                        introduction purpose. </p>
                    <p>For live preview, please click <a href="#" class="text-blue">here</a>.</p>
                </div>
            </div>
            <br/>
            <div class="row">
                <div class="col-sm-4">
                    <img src="{{asset('img/Programmer Needs-76.jpg')}}" class="img-full m-t-md">
                </div>
                <div class="col-sm-8">
                    <h2>Animation Story Video</h2>
                    <p>Explain with simulation method to tell audience about the brand story or business DNA, suitable
                        for young & funky brands. </p>
                    <p>For live preview, please click <a href="https://www.youtube.com/embed/zDZFcDGpL4U" class="text-blue js-show-video js-show-video-1" data-target="#mediaModal1" data-toggle="modal" >here</a>.</p>
                </div>
            </div>
            <br/>
            <div class="row">
                <div class="col-sm-4">
                    <img src="{{asset('img/Programmer Needs-77.jpg')}}" class="img-full m-t-md">
                </div>
                <div class="col-sm-8">
                    <h2>White Board Animation Video</h2>
                    <p>Explain with sketch drawing style or puzzle up paper cutting. It is straight forward on
                        explaining corporate introduction, suitable for companies in both education field and technical
                        field.</p>
                    <p>For live preview, please click <a href="https://www.youtube.com/embed/CzgQBAWIh_Y" class="text-blue js-show-video js-show-video-2" data-target="#mediaModal1" data-toggle="modal" >here</a>.</p>
                </div>
            </div>
            <br/>
            <br/>
            <br/>
            <br/>
            <br/>
        </div>
    </div>

    <div aria-labelledby="myModalLabel" class="modal fade" id="mediaModal1" role="dialog" tabindex="-1">

        <div class="modal-dialog homevideo" role="document">
            <div class="modal-content">
                <div class="modal-header text-right">
                    <a aria-label="Close" class="custom-close" data-dismiss="modal" type="button">
                        <img src="{{url("/img/Programmer Needs-26.svg")}}" width="50px">
                    </a>
                </div>

                <div class="modal-body">
                </div>
            </div>
        </div>
    </div>

    <div class="content-2 hide">
        <iframe allowfullscreen="" frameborder="0" height="400px"
                src="https://www.youtube.com/embed/zDZFcDGpL4U" width="100%"></iframe>
    </div>
    <div class="content-1 hide">
        <iframe allowfullscreen="" frameborder="0" height="400px"
                src="https://www.youtube.com/embed/CzgQBAWIh_Y" width="100%"></iframe>
    </div>

@endsection