@if ($randomPosts->count() >= 3)
    <style>
        .post_0_background {
            background: url('{{$randomPosts[0]->coverImage->dir}}') !important;
    ...
    <div class="col-xs-6 col-xs-push-6 wrapper__right">
        <div class="wrapper">
                ...
                <a href="{{url('insights/detail/'.$posts[1]->slug)}}">
                    <div class="content-wrapper"
                        ...
                        <div class="col-xs-11 col-md-7 content-wrapper__content text-white">
                            <?php
                            $content = html_entity_decode($randomPosts[1]->translate(Session::get('locale'))->description);
                            $content = trim(strip_tags(preg_replace("/<img[^>]+\>/i", " ", $content)));
                            ?>
                            <h2 class=" mobile-h1 text-uppercase">{{$randomPosts[1]->translate(Session::get('locale'))->title}}</h2>
            ...
            <div class="row content-wrapper-layer content-wrapper-second-layer">
                <a href="{{url('insights/detail/'.$randomPosts[2]->slug)}}">
                    <div class="content-wrapper"
                        ...
                        <div class="col-xs-11 col-md-7 content-wrapper__content text-black">
                            <?php
                            $content = html_entity_decode($randomPosts[2]->translate(Session::get('locale'))->description);
                            $content = trim(strip_tags(preg_replace("/<img[^>]+\>/i", " ", $content)));
                            ?>
                            <h2 class=" mobile-h1 text-uppercase">{{$randomPosts[2]->translate(Session::get('locale'))->title}}</h2>
                            <p class=" hidden-xs">{{mb_substr($content,0,$char_count)}}</p>
